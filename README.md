...
## 📑 회고
각 레이어에 대한 테스트를 모두 작성하다 보니, 많은 테스트 코드의 양이 많아져 시간이 많이 소요되는 어려움이 있었습니다.

하지만 테스트 코드를 작성하는 과정에서 리팩토링에 대한 자신감을 얻을 수 있었습니다. 테스트가 충분히 작성되어 있는 경우, 코드 수정 시 기존 기능이 의도치 않게 깨지지 않았는지 빠르게 검증할 수 있었기 때문입니다.

또한 테스트를 통해 유효성 검사가 제대로 동작하는지 여러 상황에서 확인하며, 발생할 수 있는 예외 케이스들을 더 꼼꼼하게 다룰 수 있었습니다.

TDD 실습을 통해 도메인 로직은 테스트 가능한 형태로 분리되어야 한다는 점과, Mock을 활용한 서비스 테스트가 팀 협업 효율을 높일 수 있다는 점을 깨달을 수 있었습니다.
